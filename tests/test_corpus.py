...
def seeds():
    """Parsed once for the module.

    ``load_seeds()`` re-reads and re-parses a 22KB YAML file on every call, and
    these tests only ever read the result. The tests below that exercise
    ``seeds_by_category`` itself still call it directly.
    """
    return load_seeds()
...
def test_every_category_has_at_least_three_seeds(seeds):
    counts = Counter(seed.category for seed in seeds)
    for category in CATEGORIES:
        assert counts[category] >= 3, f"{category} has only {counts[category]} seeds"


def test_seed_ids_are_unique(seeds):
    ids = [s.id for s in seeds]
    assert len(ids) == len(set(ids))


def test_code_judged_attacks_have_a_signal(seeds):
    for seed in seeds:
        if "code" in seed.judge:
            assert seed.success_signal is not None, seed.id


def test_llm_judged_attacks_have_an_objective(seeds):
    for seed in seeds:
        if seed.judge != "code":
            assert seed.objective.strip(), seed.id


def test_file_delivery_has_payload(seeds):
    for seed in seeds:
